...
hist_Hemoglobina<- ggplot(df, aes(x = Hemoglobina)) +
  ...
  labs(title="Histogram Hemoglobina", x="Wartocci", y="Gestocs") +
  theme_minimal()
...
ggplot(df, aes(x = LVEF, y = Predictions)) +
  ...
  labs(title = "Wykres zale?no?ci mi?dzy LVEF a Statin",
       x = "LVEF",
       y = "Statin") +
  theme_minimal()
...
ggplot(df, aes(x = NYHA, y = Predictions)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, color = "blue") +
  labs(title = "Wykres zale?no?ci mi?dzy NYHA a Statin",
       x = "NYHA",
       y = "Statin") +
  theme_minimal()
...
ggplot(df, aes(x = NT_proBNP, y = Predictions)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, color = "blue") +
  labs(title = "Wykres zale?no?ci mi?dzy NT_proBNP a Statin",
       x = "NT_proBNP",
       y = "Statin") +
  theme_minimal()

kruskal.test(NYHA~Statin, data=df) #istniej? r??nice
kruskal.test(LVEF~Statin, data=df) #nie istniej? r??nice
kruskal.test(NT_proBNP~Statin, data=df) #istniej? r??nice
